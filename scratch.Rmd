...
farming_sim <- accumulate(1:nsim, ~household_dynamics(.x, rainfall_c = 1, cultivable_area_c = 10) %>% population_dynamics, .init = test_pop2)
```
...
nsim = 600
farming_sim2 <- 1:50 %>%
  future_map(~ accumulate(1:nsim, ~household_dynamics(.x, rainfall_c = .7, cultivable_area_c = 10) %>% population_dynamics, .init = test_pop2), .progress = TRUE) %>%
...
capacity <- farming_sim2 %>%
  ...
  pull(population) %>%
  mean
logistic_growth <- accumulate(1:nsim, ~.x * (1 + 0.0155 * (1 - (.x / capacity))), .init = 30) %>%
...
farming_sim2 %>%
  filter(time %in% c(5, 50, 223, 500), rep == 1) %>%
