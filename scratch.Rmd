...
system.time(create_individuals(occupants = 10000000) %>% population_dynamics())
```
...
```{r, eval = FALSE}
temp <- reduce(1:20, ~settlement_dynamics(.x), .init = settlements)
...
unnest(unnest(as_tibble(temp), households))
```
