...
t1 <- test[[nsim]] %>%
  ...
  mutate(n_migrants = map_dbl(households, ~sum(.$migrant)),
         migrants = map(households, ~filter(., migrant == TRUE) %>% select(-migrant) %>% mutate(household = NA, land = 0, storage = 0)))
...
t3 / sum(t3) * 5
round_preserve_sum(t3 / sum(t3) * 5)
```
```{r}
