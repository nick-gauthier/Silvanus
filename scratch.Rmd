...
```{r, eval = FALSE}
profvis(reduce(1:20, ~settlement_dynamics(.x), .init = settlements))
```

```{r, cache = TRUE}
test_grid <- accumulate(1:500, ~settlement_dynamics(.x), .init = filter(activate(settlements, 'edges'), distance < 40))
```
