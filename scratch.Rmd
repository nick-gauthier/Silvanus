...
farming_sim <- accumulate(1:nsim, ~household_dynamics(.x, precipitation_c = 0.7, cultivable_area_c = 1) %>% population_dynamics, .init = test_pop2)
```
...
```{r}
logistic_growth <- accumulate(1:500, ~.x * (1 + 0.017 * (1 - (.x / 150))), .init = 9) %>%
