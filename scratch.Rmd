...
test_pop <- create_settlement(4, n_households = 4)
```
...
so we have a tibble of the migrant pool from our settlement
we know how many people are moving from our settlement to each target
so we should randomly sample n rows from the migrant table
  remove those rows from source settlement and migrant pool, add to destination
